	...
	if (checkType == "playlist") {
		playlist.forEach((song) => {
          ...
          <p>${i}</p>
            ...
            <div class="col-4">
              <h3 onclick="cercaMp3(${song.id})" class="fw-bold">${
				song.title
			}</h3>
              <h3 onclick="cercaArtista(${song.artist.id})" class=" fw-light">${
				song.artist.name
			}</h3>
            </div>
            <div class="col-3">
              <p onclick="cercaAlbum(${song.album.id})" class="text-center ">${
				song.album.title
			}</p>
            </div>
            <div class="col-3">
              <p class="text-center">${numeroAscoltoRandom}</p>
            </div>

            <div class="col-1">
              <p class="text-center">${minutaggio(song.duration)}</p>
           </div>
    	`;
			i++;
		});
